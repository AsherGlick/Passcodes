...
DomainSettings SettingsAPI::GetSettings(std::string domain, int version) {
    ...
    if (returnCode) {
        ...
        if (returnCode == 1) {
            // Create the database using the autogenerated query in DomainSettings
            int dbCreateResultCode = sqlite3_exec(this->database, DomainSettings::CreateTableSQL().c_str(), NULL, NULL, NULL);
            if (dbCreateResultCode) {
                fprintf(stderr, "Error Creating Table: %i: %s\n", dbCreateResultCode, sqlite3_errmsg(this->database));
            }
            // if the table was sucessfully created then run this command again to get the data from the table
            else {
                return GetSettings(domain, version);
            }
        }
